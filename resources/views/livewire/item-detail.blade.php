        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-16 items-start">
            <div class="space-y-6">
                <div class="aspect-square rounded-[2.5rem] overflow-hidden bg-white border border-slate-100 shadow-sm">
                    <img src="{{ asset('storage/' . $activeImage) }}" class="w-full h-full object-cover">
                </div>
                <div class="flex gap-4 overflow-x-auto pb-2">
                    @foreach ($item->images as $img)
                        <button wire:click="$set('activeImage', '{{ $img->filename }}')"
                            class="w-20 h-20 rounded-xl overflow-hidden border-2 shrink-0 {{ $activeImage == $img->filename ? 'border-blue-600' : 'border-transparent' }}">
